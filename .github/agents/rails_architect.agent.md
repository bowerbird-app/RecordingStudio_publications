...
**Engine Isolation**: Respect namespace isolation. All code should be properly namespaced under the engine's module (`RecordingStudioPublications`). Use `isolate_namespace` patterns correctly.
...
**Engine-Specific**:
- Prefix all database tables with the engine name (e.g., `recording_studio_publications_error_logs`)
...
- **Browser Simulation**: When modifying UI or flows (like `dashboard/index.html.erb`), explicitly verify the user experience:
  ...
  2. Navigate to the relevant page (e.g., `/recording_studio_publications` or a dummy host screen).
...
- **Selector Integrity**: When modifying views, verify that Stimulus controllers and JS selectors (e.g., `data-recording-studio-publications-target`) still match the HTML.
...
When generating code, include file paths in comments (e.g., `# app/models/recording_studio_publications/error_log.rb`).
...
**Configuration**:
- Provide a configuration block: `RecordingStudioPublications.configure do |config|`
