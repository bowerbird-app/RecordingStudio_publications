...
module RecordingStudioPublications
  module Generators
    class InstallGenerator < Rails::Generators::Base
      ...
      desc "Installs RecordingStudioPublications engine into your application"

      class_option(
        :mount_path,
        type: :string,
        default: "/recording_studio_publications",
        desc: "Route prefix used when mounting the engine"
      )

      def mount_engine
        route %(mount RecordingStudioPublications::Engine, at: "#{options[:mount_path]}")
      end

      def copy_initializer
        template "recording_studio_publications_initializer.rb", "config/initializers/recording_studio_publications.rb"
      end

      def add_yaml_config
        prompt = "Would you like to add `config/recording_studio_publications.yml` " \
                 "for environment-specific settings? [y/N]"
        return unless yes?(prompt)

        template "recording_studio_publications.yml", "config/recording_studio_publications.yml"
      end
      ...
      def add_tailwind_source
        ...
        if missing_lines.empty?
          say "Tailwind already configured to include RecordingStudioPublications and FlatPack sources.", :green
          return
        end
      ...
      def inject_tailwind_sources(tailwind_css_path, missing_lines)
        ...
        say "Added RecordingStudioPublications and FlatPack sources to Tailwind CSS configuration.", :green
        say "Run 'bin/rails tailwindcss:build' to rebuild your CSS.", :green
      end

      def formatted_tailwind_source_block(missing_lines)
        [
          "\n/* Include RecordingStudioPublications engine views for Tailwind CSS */",
      ...
      def tailwind_source_lines
        [
          '@source "../../vendor/bundle/**/recording_studio_publications/app/views/**/*.erb";',
          "@source \"../../../../../../usr/local/bundle/ruby/**/bundler/gems/" \
          "recording_studio_publications-*/app/views/**/*.erb\";",
